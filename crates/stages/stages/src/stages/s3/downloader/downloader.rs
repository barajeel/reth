...
/// ## Details
///
/// 1) A [`Metadata`] file is created or opened in `target_dir/download`. It tracks the download progress including total file size, downloaded bytes,
///   chunk sizes, and ranges that still need downloading. Allows for resumability. 
/// 2) The target file is preallocated with the total size of the file in `target_dir/download/filename`.
/// 3) Multiple `workers` are spawned the downloading of specific chunks of the file.
/// 4) `Orchestrator` manages workers, distributes chunk ranges, and ensures the download
///   progresses efficiently by dynamically assigning tasks to workers as they become available.
/// 5) Once the file is downloaded:
///     * If Some(file_hash) verifies its blake3 hash.
